#!/usr/bin/env bash
set -Eeo pipefail
# global iterator -- stop script from running multiple times...
...
docker_setup_env() {
        SCRIPT_RUN=1 
	echo "Current value of global test variable(docker_setup_env): $SCRIPT_RUN"
...
pg_setup_hba_conf() {
	...
	# https://www.postgresql.org/about/news/postgresql-14-released-2318/
        SCRIPT_RUN=1 
	echo "Current value of global test variable(pg_setup_hba_conf): $SCRIPT_RUN"
	if [ "$1" = 'postgres' ]; then
		shift
	fi
	local auth
...
docker_temp_server_start() {
        SCRIPT_RUN=1 
	echo "Current value of global test variable(docker_temp_server_start): $SCRIPT_RUN"
	if [ "$1" = 'postgres' ]; then
		shift
	fi
...
_main() {
	#echo "Current value of global test variable(main): $SCRIPT_RUN"
	#if [ $SCRIPT_RUN -eq 0 ]; then 
	#	echo "Boolean if-block works as expected(main):"
	#	return 1
	#fi
